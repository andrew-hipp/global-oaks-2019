...
if(!exists('tr.singletonsByLoc')) {
  tr.singletonsByLoc <- lapply(dir('allLociPhy_2019-02-19/bestTrees/', full = T), function(x) {
    read.tree(x) %>% di2multi(tol = 1E-05) })
  names(tr.singletonsByLoc) <- dir('allLociPhy_2019-02-19/bestTrees/') %>%
...
if(!'squashedTrees' %in% dir('allLociPhy_2019-02-19')) {
  dir.create('allLociPhy_2019-02-19/squashedTrees')
  lapply(names(tr.singletonsByLoc),
          function(i) {
            write.tree(tr.singletonsByLoc[[i]],
                       paste('allLociPhy_2019-02-19/squashedTrees/RAxML_bestTree.squashed',
                              i, 'tre', sep = '.'))
                     } # close fct(i)
                   ) # close lapply
                 } # close if
...
if(!exists('tr.singletonsByLoc.q')) {
  tr.singletonsByLoc.q <- SharedQuartetStatus(tr.singletonsByLoc, tr.singletons)
