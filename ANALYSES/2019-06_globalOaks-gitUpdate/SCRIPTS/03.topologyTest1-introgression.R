...
constraints <- list(
  albae.mono = list(
    albae = row.names(radMetaOut)[match(unlist(taxa[c('albae', 'roburoid')]), radMetaOut$tip)],
    ponticae = row.names(radMetaOut)[match(unlist(taxa$ponticae), radMetaOut$tip)]
  ),
  albae.intr = list(row.names(radMetaOut)[match(unlist(taxa[c('pontica', 'roburoid')]), radMetaOut$tip)]),
  dumos.mono = list(dumosae = row.names(radMetaOut)[match(unlist(taxa$dumosae), radMetaOut$tip)],
                    macrocarpae = row.names(radMetaOut)[match(unlist(taxa$macrocarpae), radMetaOut$tip)]
                  ),
  dumos.int1 = list(row.names(radMetaOut)[match(unlist(taxa[c('dumosae', 'macrocarpa')]), radMetaOut$tip)]),
  dumos.int2 = list(row.names(radMetaOut)[match(unlist(taxa[c('macrocarpae', 'lobata')]), radMetaOut$tip)])
)

constraintTreesFull <- sapply(constraints, makeConstraint,
