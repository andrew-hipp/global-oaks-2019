...
refreshEvents <- TRUE
  ...
  tr.calibrated.supplement <- read.tree(
    '../BAMM.WORKING/stem/tr.singletons.correlated.1.taxaGrepStem.tre'
    )
  ...
  write.csv(tr.quickLabeler, '../OUT/ANALYSIS.PRODUCTS/orderedSingletonTreeTips-SUPPLEMENT.csv')
    events <- getEventData(tr.calibrated.supplement, '../BAMM.WORKING/stem/event_data.txt', burnin = 0.05)
...
## get probability of changes by node
  ...
  events.p <-
    ...
    pdf(paste('../OUT/SUPPLEMENTS/FIG.S3c.phyloplot.', treePalLabel, '.pdf', sep = ''), 8.5, 11,
        useDingbats = FALSE)
